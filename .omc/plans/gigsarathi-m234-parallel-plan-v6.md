# Gigsarathi M2/M3/M4 Parallel Launch Plan (v6 — Post Critic v3 ITERATE)

## Plan Status
- Planner: v1, v2, v3, v4, v5, v6
- Architect: v1 ITERATE → v2 APPROVE → v3 ITERATE → v4 APPROVE → v5 APPROVE
- Critic: v1 ITERATE → v2 ITERATE → v3 ITERATE → v4 APPROVE

---

## Changes from v5 (Critic v3 findings — all addressed)

1. **`BotMessageController` phantom class (Major #1)**: Class does not exist. Actual inbound controller is `MessageController` at `com.gigsarathi.adapter.MessageController`. Renamed throughout plan. Added precise call site: `MessageController.ingest()`, AFTER idempotency check, BEFORE `flowEngine.handle()` invocation, with wrapping try/catch so referral-code failure does not block message ingestion.
2. **FlowEngine call site forward-defensive (Major #2)**: Plan now explicitly notes that the `FlowEngine.handle()` call site is forward-defensive — guards against future direct callers of `FlowEngine` not going through `MessageController`. Not load-bearing for MVP (single caller confirmed). Idempotency placement specified: AFTER idempotency check so duplicate-message retries do not trigger referral code generation.
3. **`application-test.yml` explicit M4 deliverable (Minor #1)**: Added explicit line item in M4 deliverable 4 and M4 AC to add `app.bootstrap.enabled: false` to `src/test/resources/application-test.yml` in the same commit as `AppConfigBootstrap`.
4. **Prep-commit gate: MVP plan banner (Ambiguity #1)**: Added gate item requiring `gigsarathi-mvp-build-plan.md` supersession banner to be updated to point to v6 plan.
5. **PO sign-off wording clarified (Ambiguity #2)**: Single-winner is per daily-summary delivery; referral is re-evaluated on each subsequent day.
6. **Null-key-return test added to M2 AC (Missing gap)**: When no `hotness:{zoneId}:*` keys exist, zone write completes without exception.
7. **SessionState roundtrip integration test added to 0b (Missing gap)**: `SessionService.saveSession()` + `getSession()` roundtrip; verify `previousFlow` survives.
8. **Test class file names specified (Missing gap)**: `PostDailySummaryActionTest.java` (Mockito tests 1–5) and `PostDailySummaryActionSpringOrderTest.java` (Spring slice test 6).
...
**Unit tests required in prep commit:**

File: `PostDailySummaryActionTest.java` (Mockito-based, tests 1–5)
...
File: `PostDailySummaryActionSpringOrderTest.java` (Spring slice test 6)
6. `@SpringJUnitConfig`: define two `@Component PostDailySummaryAction` beans with `@Order(10)` and `@Order(20)`; inject `List<PostDailySummaryAction>`; assert index 0 is the `@Order(10)` implementation. This is the only test that catches a forgotten `@Order` annotation — tests 1–5 use hand-constructed lists.
...
- [ ] `PostDailySummaryActionTest.java` (tests 1–5, Mockito) passes
- [ ] `PostDailySummaryActionSpringOrderTest.java` (test 6, Spring slice) passes
...
- [ ] Integration test using `SessionService.saveSession()` then `getSession()` round-trip; verify `previousFlow` survives roundtrip end-to-end through the service layer
...
- [ ] 0a: `PostDailySummaryAction` interface with Javadoc + explicit `for`-`break` refactor in `DailyEarningsFlow` + `DailyEarningsFlowTest.java` (M1 path) + 6 unit tests (`PostDailySummaryActionTest.java` tests 1–5 + `PostDailySummaryActionSpringOrderTest.java` test 6)
- [ ] 0b: `SessionState.previousFlow` + `@JsonIgnoreProperties(ignoreUnknown=true)` + forward-compat test + rollback-behavior test + `SessionService` roundtrip integration test + `SessionService` comment
...
- [ ] `gigsarathi-mvp-build-plan.md` supersession banner updated to reference v6 plan (crossing out M2 deliverable #5)
...
- [ ] Cache invalidation null-key test: when no `hotness:{zoneId}:*` keys exist, zone write completes without exception (verifies `null` guard in snippet)
...
4. **Referral code lazy migration** — `ReferralService.ensureReferralCode(userId)` called from two sites:
   - `MessageController.ingest()` (at `com.gigsarathi.adapter.MessageController`) — AFTER the idempotency check (`idempotencyRepository.existsByPlatformAndMessageId()`) passes, BEFORE `flowEngine.handle()` invocation (currently line ~59). Wrapped in try/catch; on failure log WARN and continue — referral code generation failure must NOT block message ingestion.
   - `FlowEngine.handle()` (at `com.gigsarathi.flow.FlowEngine`) — at the top of the method, before state transition. **This site is forward-defensive**: in MVP, `FlowEngine.handle()` has exactly one caller (`MessageController.ingest()`), so both sites are redundant by construction. The `FlowEngine` site guards against future direct callers of `FlowEngine` that bypass `MessageController`. Both sites are idempotent (no-op if code already exists).
...
- [ ] `ReferralService.ensureReferralCode(userId)` called from `MessageController.ingest()` AFTER idempotency check, BEFORE `flowEngine.handle()`; wrapped in try/catch; message ingestion continues on failure
- [ ] `ReferralService.ensureReferralCode(userId)` called from `FlowEngine.handle()` (forward-defensive site); idempotent
...
- **Reads**: `EarningsRecord`, `User` (M1); `SessionState.previousFlow` (prep commit); `FlowType.REFERRAL` (prep commit); `MessageController.ingest()` (M1, call site addition only); `FlowEngine.handle()` (M1, call site addition only)
...
3. **`PATCH /admin/features`** — toggles `loanEnabled`, `referralRewardEnabled` in `app_config`; `X-Admin-Key` auth (automatically enforced by `AdminKeyInterceptor` via `WebMvcConfig.addPathPatterns("/admin/**")`)
4. **`app_config` bootstrap** — `@EventListener(ApplicationReadyEvent.class)` upserts defaults on startup if absent. Bean guarded by `@ConditionalOnProperty` so tests can opt out:
   ...
   `application-test.yml` MUST have `app.bootstrap.enabled: false` added in the same commit as `AppConfigBootstrap`. `matchIfMissing=true` means production requires no property.
...
- [ ] `app.bootstrap.enabled: false` added to `src/test/resources/application-test.yml` in same commit as `AppConfigBootstrap`
...
| `MessageController.java` | LOW | LOW | M3 adds one call site (idempotency-safe, after idempotency check); no other branch touches it |
| `FlowEngine.java` | LOW | LOW | M3 adds forward-defensive call site; no other branch touches it |
| `ZoneController.java` | LOW | LOW | M2 adds cache-invalidation call in try/catch; no other branch touches it |
| `AppConfigBootstrap` (M4 new) | NONE | LOW | `@ConditionalOnProperty` + `app.bootstrap.enabled=false` in test yml |
...
| `application-test.yml` | LOW | LOW | M4 adds `app.bootstrap.enabled=false` in M4 commit; no other branch touches it |
...
```
main ──[ 0a+0b+0c+0d + DailyEarningsFlowTest + mvn test green + 11 M1 gates + MVP plan banner updated ]──►
```
...
5. Replace `redisTemplate.keys("hotness:*")` with SCAN-cursor approach in `ZoneController.upsert()` when shared Redis hosts more than the `hotness:*` namespace, or before a second non-MVP service joins the Redis instance
...
- [ ] `PostDailySummaryActionTest.java` (tests 1–5, Mockito) + `PostDailySummaryActionSpringOrderTest.java` (test 6, Spring `@Order` slice) pass
- [ ] `SessionState.previousFlow` + `@JsonIgnoreProperties(ignoreUnknown=true)` + forward-compat test + rollback-behavior test + `SessionService` roundtrip integration test + `SessionService` comment
...
- [ ] `gigsarathi-mvp-build-plan.md` supersession banner updated to reference v6 plan (M2 deliverable #5 crossed out)
...
- [ ] **PO acknowledgement**: users eligible for BOTH tomorrow plan (M2 `@Order(10)`) AND referral (M3 `@Order(20)`) receive ONLY tomorrow plan **for that day's daily-summary chain**. Referral prompt is re-evaluated independently on each subsequent day's daily-summary delivery — if the user no longer qualifies for tomorrow plan, referral may fire. If both should fire in sequence within a single daily-summary delivery, the `PostDailySummaryAction` interface contract (single winner per chain) must change before M2 or M3 code is written.

**Scope acknowledgements (written):**
- [ ] M2: `PATCH /admin/features` is NOT M2 scope; no `FlowType` enum edits on branch
- [ ] M3: no `HotnessScoreService` import; no `FlowType` enum edits; no v1.5 implied; referral code call sites are in `MessageController` and `FlowEngine` (not `BotMessageController`)
- [ ] M4: `PATCH /admin/features` IS M4 scope; owns `app_config`; no `FlowType` enum edits; `application-test.yml` updated in M4 commit
...
**Scenario 2: Action ordering → M3 referral never fires** → Mitigated by `@Order(10)`/`@Order(20)` Javadoc contract + `PostDailySummaryActionSpringOrderTest.java` (catches forgotten `@Order` before branching). Single-winner is per daily-summary delivery; referral re-evaluated next day.
...
**Scenario 4: M4 `@EventListener` corrupts test context** → `@ConditionalOnProperty` + `app.bootstrap.enabled=false` added to `application-test.yml` in M4 commit. `GigsarathiApplicationTests` passes without MongoDB write.

**Scenario 5: Cache invalidation takes Redis down** → try/catch around `redisTemplate.keys()`+`delete()`. Zone write API returns 200; stale hotness scores persist up to 1h TTL; logged as WARN. Null-key guard confirmed by unit test.
